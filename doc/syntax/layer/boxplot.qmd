...
* `orientation`: The orientation of the layer, either `"x"` or `"y"`. The default is based on which of the `x` and `y` aesthetics is discrete and continuous, and falls back to `"x"` if the aesthetics have a similar type.
