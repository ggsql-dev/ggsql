...
/// Apply remappings: rename stat columns and add literal columns.
///
/// After stat transforms, columns like `__ggsql_stat_count` need to be renamed
/// to the target aesthetic's prefixed name (e.g., `__ggsql_aes_y__`).
/// For literal values (e.g., `ymin=0`), this creates a constant column.
///
/// Note: Most column renames happen in the stat SQL query itself.
/// This handles any edge cases and all literal remappings.
pub fn apply_remappings_post_query(df: DataFrame, layer: &Layer) -> Result<DataFrame> {
    ...
    for (target_aesthetic, value) in &layer.remappings.aesthetics {
        ...
        match value {
            AestheticValue::Column { name, .. } => {
                // Apply column remapping: stat columns → prefixed aesthetic names
                // Note: Most renames happen in the stat SQL query itself
            ...
            AestheticValue::Literal(lit) => {
                // Add literal column from remapping
                // Only add if column doesn't already exist
                if df.column(&target_col_name).is_err() {
